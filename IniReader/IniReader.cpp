...
bool IniReader::OpenFile(const char* Filepath, int flag) {
  ...
  if (Lines.size() && Applines.size()) return true;
...
bool IniReader::Close() {
  for (std::string str : Lines) file << str.c_str() << std::endl;
...
bool IniReader::WriteStrings(std::string AppName,
...
std::vector<std::string> IniReader::GetStrings(std::string AppName) {
  ...
  if (!Lines.size() || !Applines.size()) return temp;
  ...
  for (size_t appname = 0; appname < Applines.size(); appname++) {
    ...
    if (!strcmp(str.c_str(), Applines.at(appname).Name.c_str())) {
      for (size_t key = Applines.at(appname).Index + 1; key < Lines.size();
           key++) {
        std::string full_str_temp = Lines.at(key);
        if (full_str_temp.at(0) == '[') return temp;
...
std::string IniReader::GetString(std::string AppName, std::string KeyName) {
  if (!Lines.size() || !Applines.size()) return std::string();
  ...
  for (size_t appname = 0; appname < Applines.size(); appname++) {
    ...
    if (!strcmp(str.c_str(), Applines.at(appname).Name.c_str())) {
      for (size_t key = Applines.at(appname).Index + 1; key < Lines.size();
           key++) {
        std::string full_str_temp = Lines.at(key);
        if (full_str_temp.at(0) == '[') return std::string();
        int offset = full_str_temp.find('=');
        std::string key_name = full_str_temp.substr(0, offset);
...
bool IniReader::WriteString(std::string AppName,
                            std::string KeyName,
                            std::string Value) {
  for (size_t appname = 0; appname < Applines.size(); appname++) {
    std::string str = std::string("[") + AppName.c_str() + "]";
  ...
  Applines.insert(Applines.end(),
                  AppIndex(Lines.size(),
                           (std::string("[") + AppName.c_str() + "]").c_str()));
